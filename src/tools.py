...
def gen_dx_bx(xbound, ybound, zbound):
    """
        grid_conf = {
        'xbound': xbound, #xbound=[-50.0, 50.0, 0.5],
        'ybound': ybound,#ybound=[-50.0, 50.0, 0.5],
        'zbound': zbound, #zbound=[-10.0, 10.0, 20.0],
        'dbound': dbound, #dbound=[4.0, 45.0, 1.0],
...
def cumsum_trick(x, geom_feats, ranks):
    x = x.cumsum(0)#特征在68527维度 累加[68527,64]
    kept = torch.ones(x.shape[0], device=x.device, dtype=torch.bool)
    #kept.shape:[68527],data=[True,...,True]
    kept[:-1] = (ranks[1:] != ranks[:-1])
    #ranks 后一位和前一位不同，kept 为 True
    
    x, geom_feats = x[kept], geom_feats[kept]
    #取后一位和前一位不同位置的x.geom_feats.
    x = torch.cat((x[:1], x[1:] - x[:-1]))
    # x 返回Bev栅格图中同一位置累积和最大的差值。
    return x, geom_feats
...
class QuickCumsum(torch.autograd.Function):
    ...
    def forward(ctx, x, geom_feats, ranks):
        ...
        print("kept:",kept)
        kept[:-1] = (ranks[1:] != ranks[:-1]) #当前位置上值和后一个不同为1，相同为0
        # print("kept.shape:",kept.shape)
        # print(ranks[1:]) #[  8836,   8836,   8837,
        # print(ranks[:-1]) # [  8836,   8836,   8836
        # print(kept) # [False, False,  True,
        x, geom_feats = x[kept], geom_feats[kept] #不同提取出来
        x = torch.cat((x[:1], x[1:] - x[:-1])) #x-> x梯度（后一个-前一个）
